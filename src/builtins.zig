...
// `objectGetOwnPropertyNames`/`objectGetPrototypeOf` are `pub` in
// object_builtins.zig and reached back here (aliased below) since
// Reflect (still in this file, not yet its own domain) calls them
// directly. `ownEnumerableKeys`/`freeOwnedKeys` stay re-exported since
